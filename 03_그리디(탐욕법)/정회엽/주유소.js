...
문제 설명 : 어떤 나라에 N개의 도시가 있다. 이 도시들은 일직선 도로 위에 있다. 
...
만약 제일 왼쪽 도시에서 2리터의 기름을 넣고(2×5 = 10원) 다음 번 도시까지 이동한 후 3리터의 기름을 넣고(3×2 = 6원) 다음 도시에서 1리터의 기름을 넣어(1×4 = 4원)
 제일 오른쪽 도시로 이동하면, 총 비용은 20원이다. 또 다른 방법으로 제일 왼쪽 도시에서 2리터의 기름을 넣고(2×5 = 10원) 다음 번 도시까지 이동한 후 4리터의 기름을 넣고(4×2 = 8원)
...
function solution(N, distance, price) {
  let realPrice = price.slice(0, price.length - 1);
  let minPrice = Math.min(...realPrice);
  let fixPrice = 0;
  let totalPrice = 0;
  for (let i = 0; i < N - 1; i++) {
    if (realPrice[i] === minPrice) {
      fixPrice = realPrice[i];
    }
    if (fixPrice) {
      totalPrice += distance[i] * fixPrice;
    } else {
      totalPrice += distance[i] * realPrice[i];
    }
  }
  return totalPrice;
}
console.log(solution(4, [2, 3, 1], [5, 2, 4, 1]));
console.log(solution(4, [3, 3, 4], [1, 1, 1, 1]));
